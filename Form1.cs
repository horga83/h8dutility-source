    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                string[] file_list = new string[h8d_list.Length + svd_list.Length];
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (listBox1.SelectedIndex != -1)
            {
                ...
                {
                    if (listBox1.SelectedItems[i].ToString().Contains(".H8D"))
                    {
                        string file_name = label3.Text + "\\" + listBox1.SelectedItems[i];
            ...
            else
            {
                ...
                {
                    if (listBox1.Items[i].ToString().Contains(".H8D"))
                    {
                        string file_name = label3.Text + "\\" + listBox1.Items[i];
            ...
            listBox2.Items.Add(string.Format("Total Files {0}, Total Size {1} KB", FileCount, TotalSize));
            listBox2.Items.Add("");
        }
        ...
        private void ReadCPMImage(string file_name, ref BinaryReader bin_file, ref ASCIIEncoding encoding)
        {
            int fsize = 0;
            int entry_count = 0;
            int disk_file_count = 0;
            ...
            if (idx != -1)
            {
                for (int i = 0; i < listBox2.SelectedItems.Count; i++)
                {
                    ...
                    foreach (DiskFileEntry entry in DiskFileList)
                    {
                        if (entry.ListBox2Entry == idx)
                        {
                            ExtractFile(entry.DiskImageName, entry);
                            files_extracted++;
                            break;
                        }
                    }
        ...
        private void ExtractFile(string disk_image_file, DiskFileEntry disk_file_entry)
        {
            FileStream file = File.OpenRead(disk_image_file);
            BinaryReader bin_file = new BinaryReader(file);
            byte[] buf = bin_file.ReadBytes(256);
